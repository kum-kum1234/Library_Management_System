...
crow::response jsonRes(int code, crow::json::wvalue body) {
...
crow::response errRes(int code, const std::string& msg) {
...
// Stripe requires ~$0.50 USD equivalent; use ₹50 as a safe INR minimum.
...
void registerPaymentRoutes(App& app) {
    ...
    ([](const crow::request& req) {
        ...
        if (!body) return errRes(400, "Invalid JSON");
        ...
        if (transactionId <= 0) return errRes(400, "transaction_id is required");
        if (fineAmount <= 0) return errRes(400, "fine_amount must be greater than 0");

        // Stripe enforces ~₹50 minimum; charge at least that while recording actual fine owed.
        double stripeChargeAmount =
            fineAmount < STRIPE_MIN_INR ? STRIPE_MIN_INR : fineAmount;

        std::string stripeKey = stripeSecret();
        if (stripeKey.empty()) {
            return errRes(500, "STRIPE_SECRET_KEY is not configured");
        }
        ...
        if (!statusRow) {
            ...
            Database::close(conn);
            return errRes(404, "Transaction not found");
        }
        ...
        if (txStatus != "issued" && txStatus != "ISSUED" &&
            ...
            Database::close(conn);
            return errRes(409, "Transaction is not eligible for payment");
        }
        ...
        try {
            ...
            if (!session) {
                Database::close(conn);
                return errRes(502, "Invalid Stripe JSON response");
            }
            ...
            if (sessionId.empty() || checkoutUrl.empty()) {
                Database::close(conn);
                return errRes(502, "Invalid Stripe response");
            }
    ...
    ([](const crow::request& req) {
        ...
        if (paymentMethod == "UPI" || paymentMethod == "upi") {
            ...
            if (stripePaymentId.empty()) {
                Database::close(conn);
                return errRes(400, "stripe_session_id is required for Stripe verification");
            }
        ...
        if (!paid) {
            ...
            Database::close(conn);
            return errRes(402, "Payment not completed");
        }
